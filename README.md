...
### But why are we doing all of this hashing and signing stuff anyway?
When we make API requests through HTTP, the server receving the request has to consider whether an outsider modify the request. The server has to deal with the chance the request is actually coming from us, and not somebody pretending to be us. RSA is a digital signature algorithm that helps with authentication. You have a private key that you don't share with anyone and use it to encrypt messages. The server has a public key that can only decrypt messages encrypted by your private key. When receiving a request, the server recreates the signing string using the informationg given in the request and header order in the auth header, and then hashes the signing string with SHA256. When the server hashes the signing string, it should be the exact same hash as the one you calculated. The SHA256 hashing algorithm is deterministic, meaning it will always produce the same output given the same input. (Also, SHA256 produces a fixed-size hash that is usually smaller than the signing string, so it is more efficient to sign.) The server then uses your public signing key to decrypt the received signature and checks to see if the decrypted signature matches the hash it just computed. If they match, then the request has been verified. If not, then the authorization fails because the request may have been tampered with or came from an unauthorized client.
